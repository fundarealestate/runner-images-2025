...
##  Desc:  Safely disables or configures Windows Defender on Windows Server 2025 / Azure base images
##         Designed for Packer retryable provisioner support.
################################################################################
...
if ($null -eq $defenderService) {
    ...
    if ($defenderService.Status -ne 'Running') {
        ...
        try {
            ...
            Write-Warning "[Critical] Could not start WinDefend service: $($_.Exception.Message)"
            $criticalError = $true
            $hasDefender = $false
        }
    }
...
if ($hasDefender) {
    ...
    foreach ($avParams in $avPreference) {
        try {
            ...
            Write-Host " ==> Applied: $($avParams.Keys -join ', ')"
        } catch {
            Write-Warning "  (!) Skipped $($avParams.Keys -join ', '): $($_.Exception.Message)"
        }
    }
...
# Exit non-zero if a critical failure occurred
